...
import { routes, fareRoutes, fleet, site } from "@/lib/data";
    ...
    <header className="sticky top-0 z-50 border-b border-sandline bg-white/95 backdrop-blur">
      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-8">
        ...
        <nav className="hidden items-center gap-8 lg:flex">
          <NavDropdown
            label="Umrah Cab"
            href="/routes"
            items={routes.map((r) => ({ slug: r.slug, label: r.label, base: "/routes" }))}
          />
          <NavDropdown
            label="Fleet"
            href="/fleet"
          ...
          <NavDropdown
            label="Pricing"
            href="/pricing"
            items={fareRoutes.map((f) => ({ slug: f.slug, label: f.label, base: "/pricing" }))}
          />
      ...
      {mobileOpen && (
        <div className="border-t border-sandline bg-white px-4 py-4 lg:hidden">
          <MobileSection title="Umrah Cab" base="/routes" items={routes} labelKey="label" />
          <MobileSection
            title="Fleet"
            base="/fleet"
            items={fleetLinks}
            labelKey="name"
          />
          <MobileSection title="Pricing" base="/pricing" items={fareRoutes} labelKey="label" />
